...
def argMaxMultiple(c):
    if len(list(c.keys())) == 0:
        return None
    all = list(c.items())
    values = [x[1] for x in all]
    max_value = max(values)
    return [key for key, value in all if value == max_value]
...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        new_actions = util.Counter()

        for action in self.getLegalActions(state):
            new_actions[action] = self.getQValue(state, action)

        if argMaxMultiple(new_actions) is None:
            return None

        return random.choice(argMaxMultiple(new_actions))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        features = self.featExtractor.getFeatures(state, action)
        error = ((reward + self.discount * self.getValue(nextState)) - self.getQValue(state, action))
        for key in features:
